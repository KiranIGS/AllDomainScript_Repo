...
public class DriverInstance extends Drivertools {
	...
	public DriverInstance(String Application, String deviceName, String portno) {
		...
		try {
			...
			switch (getPlatform()) {
			case "Android":
				DriverManager.setAppiumDriver((AppiumDriver) new AndroidDriver(
...
//				options.addArguments("--headless");
		...
		tlWebDriver.get().manage().timeouts().implicitlyWait(Duration.ofSeconds(30));
		tlWebDriver.get().manage().timeouts().pageLoadTimeout(Duration.ofSeconds(100));
	...
	public static void removePermisson(String packagename) {
		...
		try {
			...
			logger.info("**Cleared The App Data Successfully***");
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
